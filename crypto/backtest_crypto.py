...
STOP_LOSS_PCT = -0.05      # -5% (Wider than stock -4%)
TAKE_PROFIT_PCT = 10.0     # Disabled
POSITION_SIZE_PCT = 0.40   # 40% (Reduced from 60% due to volatility)
MIN_CONFIDENCE = 0.51      # Aggressive Entry
...
def run_backtest(ticker: str, period: str, device):
    ...
    for i in range(WINDOW_SIZE, len(close_prices)):
        ...
        if position is not None:
            ...
            trailing_stop_price = highest_price * (1 - 0.04) # 4.0% trailing stop (Wider for Crypto)
